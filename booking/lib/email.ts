...
export async function sendBookingConfirmationEmail(bookingData: BookingData): Promise<boolean> {
  try {
    ...
    console.log('✅ Booking confirmation email sent successfully to:', customer.email, '| ID:', data?.id);
    return true;
  } catch (error) {
    console.error('❌ Resend booking confirmation email error:', error);
    return false;
...
/**
 * Re-send a booking confirmation to a customer (triggered from Notion button)
 * Uses the same design as sendBookingUpdateEmail but with a "confirmation" tone.
 */
export async function sendBookingResendConfirmationEmail(data: BookingUpdateData): Promise<boolean> {
  if (!data.customer.email) return false;
  ...
  try {
    ...
    const { data: result, error } = await resend.emails.send({
      ...
      to:  [customer.email],
      bcc: bccEmails,
      subject: `Booking Confirmation - ${bookingId} | Memories Photography Studio`,
      html: `
        <!DOCTYPE html>
        <html>
        <head>
            ...
            <tr>
              <td align="center" style="padding: 40px 20px;">
                <table role="presentation" style="width: 600px; max-width: 100%; border-collapse: collapse; background-color: #FFFFFF; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
                  ...
                  <!-- Booking ID -->
                  <tr>
                    <td style="padding: 24px 40px 0 40px; text-align: center; background-color: #f0f8f5;">
                      <p style="margin: 0 0 4px 0; color: #666; font-size: 12px; text-transform: uppercase; letter-spacing: 1px;">Booking ID</p>
                      <p style="margin: 0 0 20px 0; color: #0b3d2e; font-size: 22px; font-weight: bold; font-family: 'Courier New', monospace;">${bookingId}</p>
                    </td>
                  </tr>
                  ...
                  <tr>
                    <td style="padding: 28px 40px 20px 40px;">
                      <h2 style="margin: 0 0 16px 0; color: #0b3d2e; font-size: 18px; border-bottom: 2px solid #0b3d2e; padding-bottom: 8px;">Session Details</h2>
                      <table style="width: 100%; border-collapse: collapse;">
                        <tr>
                          <td style="padding: 8px 0; color: #666; font-size: 14px; width: 40%;">Customer:</td>
                          <td style="padding: 8px 0; color: #2C2C2C; font-size: 14px; font-weight: 600;">${customer.firstName} ${customer.lastName}</td>
                  ...
                  <tr>
                    <td style="padding: 0 40px 28px 40px;">
                      <p style="margin: 0; color: #555; font-size: 13px; line-height: 1.6; background: #f0f8f5; border-left: 4px solid #0b3d2e; padding: 12px 16px; border-radius: 4px;">
                        📍 <strong>Location:</strong> Memories Photography Studio, Indang, Cavite<br>
                        🗺️ <a href="https://maps.app.goo.gl/kcjjzkZnvvpxJmQL9" style="color: #0b3d2e;">View on Google Maps</a>
                      </p>
                    </td>
                  </tr>
    ...
    console.log('✅ Resend confirmation email sent to:', customer.email, '| ID:', result?.id);
    return true;
  } catch (error) {
    console.error('❌ Resend confirmation email error:', error);
    return false;
...
/**
 * Send a session reminder email (triggered from Notion button)
 */
export async function sendBookingReminderEmail(data: BookingUpdateData): Promise<boolean> {
  if (!data.customer.email) return false;
  ...
  try {
    ...
    const { data: result, error } = await resend.emails.send({
      ...
      to:  [customer.email],
      bcc: bccEmails,
      subject: `Session Reminder - ${bookingId} | Memories Photography Studio`,
      html: `
        <!DOCTYPE html>
        <html>
        <head>
            ...
            <tr>
              <td align="center" style="padding: 40px 20px;">
                <table role="presentation" style="width: 600px; max-width: 100%; border-collapse: collapse; background-color: #FFFFFF; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
                  ...
                  <tr>
                    <td style="padding: 40px 40px 20px 40px; text-align: center; background-color: #0b3d2e; border-radius: 8px 8px 0 0;">
                      <h1 style="margin: 0; color: #FFFFFF; font-size: 28px; font-weight: bold;">📸 Session Reminder</h1>
                      <p style="margin: 10px 0 0 0; color: #FAF3E0; font-size: 14px;">Memories Photography Studio</p>
                    </td>
                  </tr>
                  ...
                  <tr>
                    <td style="padding: 30px 40px 10px 40px; text-align: center;">
                      <p style="margin: 0; color: #2C2C2C; font-size: 18px; line-height: 1.6;">
                        Hi <strong>${customer.firstName}</strong>! 👋<br>
                        Just a friendly reminder that your photography session is coming up soon.
                      </p>
                    </td>
                  </tr>
                  ...
                  <!-- Booking ID -->
                  <tr>
                    <td style="padding: 16px 40px; text-align: center; background-color: #f0f8f5;">
                      <p style="margin: 0 0 4px 0; color: #666; font-size: 12px; text-transform: uppercase; letter-spacing: 1px;">Booking ID</p>
                      <p style="margin: 0; color: #0b3d2e; font-size: 22px; font-weight: bold; font-family: 'Courier New', monospace;">${bookingId}</p>
                    </td>
                  </tr>
                  ...
                  <tr>
                    <td style="padding: 28px 40px 20px 40px;">
                      <h2 style="margin: 0 0 16px 0; color: #0b3d2e; font-size: 18px; border-bottom: 2px solid #0b3d2e; padding-bottom: 8px;">Your Session Details</h2>
                      <table style="width: 100%; border-collapse: collapse;">
                        <tr>
                          <td style="padding: 8px 0; color: #666; font-size: 14px; width: 40%;">Customer:</td>
                          <td style="padding: 8px 0; color: #2C2C2C; font-size: 14px; font-weight: 600;">${customer.firstName} ${customer.lastName}</td>
                        ...
                        <tr>
                          <td style="padding: 8px 0; color: #666; font-size: 14px;">📅 Date &amp; Time:</td>
                          <td style="padding: 8px 0; color: #0b3d2e; font-size: 15px; font-weight: 700;">${formattedDateTime} at ${formattedTime}</td>
                        </tr>
                        ${duration ? `<tr>
                          <td style="padding: 8px 0; color: #666; font-size: 14px;">Duration:</td>
                          <td style="padding: 8px 0; color: #2C2C2C; font-size: 14px;">${duration} minutes</td>
                  ...
                  <tr>
                    <td style="padding: 0 40px 28px 40px;">
                      <div style="background: #fffbeb; border-left: 4px solid #d4a017; padding: 16px; border-radius: 4px;">
                        <p style="margin: 0 0 8px 0; color: #2C2C2C; font-size: 14px; font-weight: bold;">📋 Before your session:</p>
                        <ul style="margin: 0; padding-left: 20px; color: #555; font-size: 13px; line-height: 1.8;">
                          <li>Arrive 5–10 minutes early</li>
                          <li>Bring any outfits or props you'd like to use</li>
                          <li>📍 <strong>Location:</strong> Memories Photography Studio, Indang, Cavite</li>
                          <li>🗺️ <a href="https://maps.app.goo.gl/kcjjzkZnvvpxJmQL9" style="color: #0b3d2e;">View on Google Maps</a></li>
                        </ul>
                      </div>
                    </td>
                  </tr>
    ...
    console.log('✅ Reminder email sent to:', customer.email, '| ID:', result?.id);
    return true;
  } catch (error) {
    console.error('❌ Reminder email error:', error);
    return false;
