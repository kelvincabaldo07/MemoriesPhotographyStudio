    ...
    <div className="space-y-6 max-w-4xl">
      <div>
        <div className="flex items-center justify-between mb-2">
          <h1 className="text-2xl font-semibold text-neutral-900">Availability</h1>
          {hasChanges && <Button onClick={saveChanges} className="bg-[#0b3d2e] hover:bg-[#0b3d2e]/90">Save</Button>}
        </div>
        ...
        <div className="p-6 border-b border-neutral-200">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2 text-sm text-neutral-600">
              <span>Timezone</span>
              <select className="border border-neutral-300 rounded-md px-3 py-1.5 text-sm bg-white">
          ...
          <div className="space-y-3">
            {Object.entries(schedule).map(([day, hours]) => (
              <div key={day} className="group">
                <div className="flex items-start gap-4 py-3 border-b border-neutral-100">
                    ...
                    {hours.enabled ? (
                      <div className="space-y-2">
                        <div className="flex items-center gap-2">
                          <Input type="time" value={hours.open} onChange={(e) => updateDay(day, "open", e.target.value)} className="w-28 h-9 text-sm" />
                          <span className="text-neutral-400">-</span>
                          <Input type="time" value={hours.close} onChange={(e) => updateDay(day, "close", e.target.value)} className="w-28 h-9 text-sm" />
                        </div>

                        {hours.breaks.map((breakSlot, idx) => (
                          <div key={breakSlot.id} className="flex items-center gap-2 pl-4">
                            <span className="text-xs text-neutral-500 w-16">{idx === 0 ? "Break" : `Break ${idx + 1}`}</span>
                            <Input type="time" value={breakSlot.start} onChange={(e) => updateBreak(day, breakSlot.id, "start", e.target.value)} className="w-28 h-8 text-sm" />
                            <span className="text-neutral-400">-</span>
                            <Input type="time" value={breakSlot.end} onChange={(e) => updateBreak(day, breakSlot.id, "end", e.target.value)} className="w-28 h-8 text-sm" />
                            <Button variant="ghost" size="sm" onClick={() => removeBreak(day, breakSlot.id)} className="h-8 w-8 p-0 text-neutral-400 hover:text-red-600">
                              <Trash2 className="w-4 h-4" />
                            </Button>
                          </div>
                        ))}

                        <Button variant="ghost" size="sm" onClick={() => addBreak(day)} className="h-8 text-xs text-[#0b3d2e] hover:text-[#0b3d2e] hover:bg-[#0b3d2e]/5">
                  ...
                  <Button variant="ghost" size="sm" onClick={() => copyDaySchedule(day)} className="opacity-0 group-hover:opacity-100 transition-opacity h-8 w-8 p-0 text-neutral-400 hover:text-[#0b3d2e]" title="Copy to all days">
      ...
      <Card className="border border-neutral-200">
        <div className="p-6 border-b border-neutral-200">
          <div className="flex items-center justify-between">
            ...
            <Button onClick={() => setShowBlockedDateForm(!showBlockedDateForm)} size="sm" className="bg-[#0b3d2e] hover:bg-[#0b3d2e]/90">
          ...
          {showBlockedDateForm && (
            <div className="mb-6 p-4 border border-neutral-200 rounded-lg bg-neutral-50">
              ...
              <div className="flex gap-2">
                <Button onClick={addBlockedDate} disabled={!newBlockedDate.startDate} className="bg-[#0b3d2e] hover:bg-[#0b3d2e]/90">Save Override</Button>
                <Button variant="outline" onClick={() => { setShowBlockedDateForm(false); setNewBlockedDate({ allDay: true }); }}>Cancel</Button>
              </div>
            </div>
          )}
